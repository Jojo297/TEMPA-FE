...
import useGetDetailVerificationCampus from "@/hooks/hooksCampus/useGetDetailVerificationCampus";
...
export default function DashboardCampusEditRegisterMitra() {
  ...
  const { isLoading, errorEditCampus, successMessage, editDataCampus } =
    useEditDataCampus();
  ...
  console.log(detailCampus);

  // 1. Fetch detail kampus saat komponen dimuat
  useEffect(() => {
    if (token) {
      fetchDetailVerificationCampus(token);
    }
  }, [token, fetchDetailVerificationCampus]);
  ...
  // 2. Fetch daftar provinsi saat komponen dimuat
  useEffect(() => {
    fetchProvince();
  }, [fetchProvince]);

  // 3. Isi form dengan data yang ada setelah detail kampus dan daftar provinsi berhasil dimuat
  useEffect(() => {
    if (detailCampus && province.length > 0) {
      const provinceData = province.find(
        (p) => p.name === detailCampus.province
      );

      form.reset({
        campusName: detailCampus.campus_name,
        emailCampus: detailCampus.email_campus,
        description: detailCampus.description,
        websiteCampus: detailCampus.website_campus,
        valueProvince: detailCampus.province,
        valueCity: detailCampus.city,
        valueSubdistrict: detailCampus.subdistrict,
        valueWard: detailCampus.ward,
        selectedLocation: { lat: detailCampus.lat, lng: detailCampus.lng },
        isCampusVerifiedByApi: true,
      });

      setSelectedLocation({ lat: detailCampus.lat, lng: detailCampus.lng });

      if (provinceData) {
        setvalueProvince(provinceData.code);
      }
    }
  }, [detailCampus, province, form.reset]);

  // 4. Rantai useEffect untuk mengambil dan mengatur data alamat berikutnya
  useEffect(() => {
    if (valueProvince) {
  ...
  // 5. Set nilai Kota saat data kota tersedia & cocokkan dengan detailCampus
  useEffect(() => {
    if (detailCampus?.city && city.length > 0) {
      const cityData = city.find((c) => c.name === detailCampus.city);
      if (cityData && valueCity !== cityData.code) {
        setvalueCity(cityData.code);
      }
    }
  }, [detailCampus, city, valueCity]);

  // 6. Fetch Kecamatan saat valueCity berubah
  useEffect(() => {
    if (valueCity) {
  ...
  // 7. Set nilai Kecamatan saat data kecamatan tersedia & cocokkan dengan detailCampus
  useEffect(() => {
    if (detailCampus?.subdistrict && subdistrict.length > 0) {
      const subdistrictData = subdistrict.find(
        (s) => s.name === detailCampus.subdistrict
      );
      if (subdistrictData && valueSubdistrict !== subdistrictData.code) {
        setValueSubdistrict(subdistrictData.code);
      }
    }
  }, [detailCampus, subdistrict, valueSubdistrict]);

  // 8. Fetch Kelurahan saat valueSubdistrict berubah
  useEffect(() => {
    if (valueSubdistrict) {
  ...
  // 9. Set nilai Kelurahan saat data kelurahan tersedia & cocokkan dengan detailCampus
  useEffect(() => {
    if (detailCampus?.ward && ward.length > 0) {
      const wardData = ward.find((w) => w.name === detailCampus.ward);
      if (wardData && valueWard !== wardData.code) {
        setValueWard(wardData.code);
      }
    }
  }, [detailCampus, ward, valueWard]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await editDataCampus(token, payload);
      if (result.success) {
      ...
      toast.error(error.message || "Gagal memperbarui data kampus.");
    }
  };
      ...
      <div className="bg-[#013B36] pt-16 pb-32 px-6 rounded-md">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-emerald-100/80 text-lg max-w-2xl mx-auto">
            Perbarui informasi profil kampus Anda untuk memastikan data tetap
            akurat.
          </p>
        </div>
      </div>
